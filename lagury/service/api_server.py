...
def add_task_local():
    ...
    _check_input_arguments(data,
                           ...
                           optional_fields=['priority', 'copy_source', 'description'])
    ...
    copy_source = data.get('copy_source', True)
    ...
    if copy_source:
        source_dir = utils.get_new_data_dir('source', create_dir=False)
        shutil.copytree(launch_dir, source_dir)
    else:
        source_dir = launch_dir

    output_node = db.DataNode(target_dir=utils.get_new_data_dir('output'))
    source_node = db.DataNode(target_dir=source_dir)
